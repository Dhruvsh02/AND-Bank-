...
import {
  ...
  PiggyBank, Eye, EyeOff, TrendingUp,
  Smartphone, RefreshCw, Copy, Check
} from 'lucide-react'
...
const StatCard = ({ label, value, sub, color, icon: Icon }) => (
  <div style={{...S.card, display:'flex', flexDirection:'column', gap:'0.5rem'}}>
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start'}}>
      <span style={{fontSize:'0.7rem', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:C.muted}}>{label}</span>
    ...
    <div style={{...S.page}}>
      ...
      <main style={{flex:1, overflowY:'auto', padding:'2rem'}}>
        ...
        <div style={{background:`linear-gradient(135deg, #0A1628 0%, #1a3a6b 50%, #0A1628 100%)`, border:`1px solid rgba(201,168,76,0.2)`, borderRadius:'1.25rem', padding:'2rem', marginBottom:'1.5rem', position:'relative', overflow:'hidden'}}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.5rem'}}>
            <div>
              ...
              <div style={{color:C.muted, fontSize:'0.875rem', fontFamily:'monospace', letterSpacing:'0.05em'}}>
                {account?.account_number || '—'}
              </div>
            </div>
            <button onClick={() => setHideBalance(h => !h)} style={{background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:'0.5rem', padding:'0.5rem', cursor:'pointer', color:C.muted}}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'1rem'}}>
            <div style={{display:'flex', alignItems:'center', gap:'0.5rem', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'0.625rem', padding:'0.5rem 0.75rem'}}>
              <Smartphone size={14} color={C.gold} />
              <span style={{color:C.muted, fontSize:'0.8rem', fontFamily:'monospace'}}>{account?.upi_id || 'No UPI ID'}</span>
              <button onClick={copyUPI} style={{background:'none', border:'none', cursor:'pointer', color:C.muted, padding:'0', display:'flex'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(160px,1fr))', gap:'1rem', marginBottom:'1.5rem'}}>
          <StatCard label="Total In"      value={fmt.currency(txns.filter(t=>t.txn_type==='credit').reduce((a,t)=>a+parseFloat(t.amount||0),0))} sub="This month" color="34,197,94"  icon={TrendingUp} />
          <StatCard label="Total Out"     value={fmt.currency(txns.filter(t=>t.txn_type==='debit').reduce((a,t)=>a+parseFloat(t.amount||0),0))}  sub="This month" color="239,68,68"  icon={ArrowUpRight} />
        ...
        <div style={S.card}>
          ...
          {txns.length === 0
            ? <div style={{textAlign:'center', padding:'3rem 0', color:C.muted}}>
                <ArrowLeftRight size={40} style={{margin:'0 auto 1rem', opacity:0.3}} />
